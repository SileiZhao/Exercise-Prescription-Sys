...
def test_llm_readiness_accepts_dashscope_key_alias() -> None:
    service = ReadinessService(
        settings=Settings(LLM_PROVIDER="aliyun", DASHSCOPE_API_KEY="test-dashscope-api-key")
    )

    status = service.check_llm()

    assert status.status == "ok"
    assert status.detail == "aliyun:qwen3.7-max"
    assert "API_KEY" not in status.detail
...
def test_embedding_readiness_accepts_dashscope_key_alias() -> None:
    ...
    assert status.status == "ok"
    assert status.detail == "dashscope:text-embedding-v4"
    assert "API_KEY" not in status.detail
    assert "key=" not in status.detail

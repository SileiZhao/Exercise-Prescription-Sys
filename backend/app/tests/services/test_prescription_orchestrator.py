...
def test_prescription_orchestrator_filters_candidate_actions_by_risk_contraindications(client, db_session, monkeypatch):
    monkeypatch.setattr("app.services.prescription_orchestrator.settings.ENVIRONMENT", "development")
    add_template(db_session, "R2")
    add_knowledge(db_session)
    safe = ExerciseAction(
        name="固定自行车低阻力",
...
def test_prescription_orchestrator_uses_reference_contraindications_to_filter_actions(client, db_session, monkeypatch, tmp_path):
    monkeypatch.setattr("app.services.prescription_orchestrator.settings.ENVIRONMENT", "development")
    docs_dir = tmp_path / "docs" / "ai_generated"
    docs_dir.mkdir(parents=True)
    (docs_dir / "ai_generated_contraindications.partial.json").write_text(
        """{"items":[{"contraindication_code":"CI_HYP_UNC_001","disease":"未控制的高血压","risk_level":"R2","contraindication_type":"relative","forbidden_action_categories":["抗阻","器械"],"stop_signals":["胸痛"],"referral_criteria":["血压持续>180/110mmHg需转介心内科"],"rationale":"高血压需限制抗阻和器械训练"}]}""",
        encoding="utf-8",
    )
    ...
    db_session.commit()
    user_id, headers = user_id_from_token(client, "rx-reference-contra@example.com")
    submit_health(client, headers, r2=True)

    record = PrescriptionOrchestrator(db_session, llm_provider=FakeAliyunLLMProvider()).generate_for_user(user_id)

    action_names = [item["name"] for item in record.llm_payload["candidate_actions"]]
    assert action_names == ["椅上呼吸训练"]
    assert record.llm_payload["contraindication_reference"]["matched_codes"] == ["CI_HYP_UNC_001"]
    assert record.llm_payload["contraindication_reference"]["forbidden_action_categories"] == ["抗阻", "器械"]
    evidence = db_session.scalar(select(PrescriptionEvidence).where(PrescriptionEvidence.prescription_id == record.id))
    assert evidence is not None
    assert evidence.safety_validation["contraindication_reference"]["matched_codes"] == ["CI_HYP_UNC_001"]

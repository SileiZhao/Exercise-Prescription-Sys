...
def test_non_r3_prescription_keeps_only_candidate_or_template_actions():
    draft = {
        "risk_level": "R1",
        "cluster_label": "体重管理型",
        "goals": ["减脂"],
    ...
    result = enforce_prescription_safety(
        ...
        candidate_action_names=["快走"],
    )

    assert result["fitt_vp"]["type"] == ["快走"]
    assert "AI自编燃脂操" not in result["fitt_vp"]["type"]


def test_non_r3_prescription_falls_back_to_template_actions_when_llm_actions_are_not_allowed():
    draft = {
        "risk_level": "R1",
        "cluster_label": "体重管理型",
        "goals": ["减脂"],
    ...
    result = enforce_prescription_safety(
        ...
        candidate_action_names=["快走", "八段锦"],
    )

    assert result["fitt_vp"]["type"] == ["快走", "八段锦"]

...
class PrescriptionOrchestrator:
    ...
    def generate_for_user(self, user_id: int) -> PrescriptionRecord:
        ...
        matched_rule_codes = [rule.code for rule in risk.matched_rules]
        contraindication_reference = ContraindicationReferenceService().match(
            risk_level=risk.risk_level,
        ...
        candidate_actions = TemplateMatchingService(self.db).candidate_actions(
            ...
            goals,
            risk_contraindications=risk.contraindications,
            forbidden_action_categories=contraindication_reference.get("forbidden_action_categories"),
        )
        ...
        forbidden_constraints.extend(contraindication_reference.get("forbidden_action_categories") or [])
        forbidden_constraints.extend(contraindication_reference.get("stop_signals") or [])
        forbidden_constraints.extend(contraindication_reference.get("referral_criteria") or [])
        llm_payload = {
            "risk_level": risk.risk_level,
            ...
            "risk_rules": [rule.model_dump() for rule in risk.matched_rules],
            "contraindication_reference": contraindication_reference,

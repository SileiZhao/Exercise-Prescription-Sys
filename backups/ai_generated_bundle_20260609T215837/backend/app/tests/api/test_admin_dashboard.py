...
def test_admin_dashboard_summary(client: TestClient, db_session):
    ...
    failed_knowledge = KnowledgeDocument(
        title="索引失败知识文档",
        category="指南",
        source="向量库",
        status="INDEX_FAILED",
        skipped_reason="向量维度不匹配",
    )
    ...
    db_session.add_all(
        [
            ...
            KnowledgeChunk(
                document_id=failed_knowledge.id,
                chunk_index=0,
                content="失败文档不应进入 ACTIVE RAG 完成率分母。",
                tags=["RAG"],
                embedding_ref=None,
            ),
        ]
    ...
    assert body["reference_data_status"]["knowledge"]["failed_documents"] == 1

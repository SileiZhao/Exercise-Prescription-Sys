...
export const designTokens = {
  ...
  shadow: {
    card: "0 8px 24px rgba(24, 44, 74, .06)"
  }
};


const launchScopeSupported = ["成人一般健康", "慢病风险管理", "体重管理", "心肺耐力", "肌力提升", "柔韧提升"];
const launchScopeRouted = ["儿童", "孕产", "术后康复", "急性病", "高危心血管事件", "R3 高风险"];

export function ClinicalScopePanel({ compact = false }: { compact?: boolean }) {
  return (
    <section className={`clinical-scope-panel${compact ? " is-compact" : ""}`} aria-label="默认上线边界">
      <div className="clinical-scope-copy">
        <span className="clinical-scope-icon"><ShieldCheck size={18} /></span>
        <div>
          <Typography.Text className="clinical-scope-title">默认上线边界</Typography.Text>
          <Typography.Text type="secondary">
            仅面向成人一般健康、慢病风险管理、体重管理与心肺/肌力/柔韧提升；高风险与特殊人群默认转专家或医疗机构。
          </Typography.Text>
        </div>
      </div>
      <div className="clinical-scope-tags">
        {launchScopeSupported.map((item) => <Tag color="green" key={item}>{item}</Tag>)}
        {launchScopeRouted.map((item) => <Tag color="red" key={item}>{item} 转介</Tag>)}
      </div>
    </section>
  );
}

export function SafetyBoundaryChecklist({ compact = false }: { compact?: boolean }) {
  return (
    <Alert
      className={`safety-boundary-checklist${compact ? " is-compact" : ""}`}
      type="warning"
      showIcon
      icon={<AlertTriangle size={18} />}
      message="上线安全边界"
      description="中风险审核通过前不展示训练动作、强度和开始入口；高风险仅展示医学评估/转介建议，不生成 FITT-VP 训练处方。"
    />
  );
}
...
export function PageHeader({
  ...
  return (
    <div className="page-header">
      <div className="page-header-title-block" data-testid="page-header-title-block">
        ...
        {subtitle ? <Typography.Text className="page-subtitle" type="secondary">{subtitle}</Typography.Text> : null}
      </div>
      <Space wrap className="workspace-meta" data-testid="page-header-meta">
        {role !== "public" ? (
          <>
...
export function AnimatedNumber({ value }: { value: string | number }) {
  ...
  const [displayValue, setDisplayValue] = useState(() =>
    parsed.numeric === null || reduced ? String(value) : formatMetricNumber(0, parsed.suffix)
  );

  useEffect(() => {
    if (parsed.numeric === null || reduced) {
    ...
    const tick = (now: number) => {
      ...
      setDisplayValue(formatMetricNumber(parsed.numeric * eased, parsed.suffix));
      if (progress < 1) {
  ...
  }, [parsed.numeric, parsed.suffix, reduced, value]);

  const animated = parsed.numeric !== null && !reduced;

  return (
    <span className="metric-number-wrap">
      <motion.span
        data-testid="count-up-number"
        data-count-up={parsed.numeric ?? undefined}
        data-motion={reduced ? "reduced" : "standard"}
      ...
      {animated ? (
        <span className="metric-final-value" data-testid="metric-final-value" aria-live="polite">
          {String(value)}
        </span>
      ) : null}
    </span>
  ...
  return (
    <Tooltip title="仅用于演示、培训、试点汇报和流程验证，不代表生产真实数据。">
      <div
        ...
        data-motion={reduced ? "reduced" : "standard"}
      >
        <span className="demo-data-banner-icon" aria-hidden="true">Demo</span>
        <div>
          <Typography.Text strong>当前为示范数据</Typography.Text>
          <Typography.Paragraph>
            仅用于演示、培训和试点汇报；数据带有 demo 标识，账号、处方、审核、反馈和科研导出均用于产品展示。
          </Typography.Paragraph>
        </div>
      </div>
    </Tooltip>

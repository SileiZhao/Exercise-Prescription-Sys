...
def _action_allowlist(
    *,
    candidate_action_names: list[str] | None,
    template_fitt_vp: dict[str, Any] | None,
) -> list[str]:
    if candidate_action_names:
        return _dedupe_strings(candidate_action_names)
    if candidate_action_names == []:
        return _dedupe_strings([str(item) for item in (template_fitt_vp or {}).get("type") or []])
    return []
...
def enforce_prescription_safety(
    ...
    *,
    ...
    if isinstance(fitt_vp, dict):
        allowed_actions = _action_allowlist(
            candidate_action_names=candidate_action_names,
            template_fitt_vp=template_fitt_vp,
        )
        if allowed_actions:
            action_types = [str(item) for item in fitt_vp.get("type") or []]
            safe_types = [action for action in action_types if action in allowed_actions]
            if not safe_types:
                safe_types = allowed_actions

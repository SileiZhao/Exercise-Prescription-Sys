...
def test_validate_reference_data_reports_formal_reference_semantic_coverage():
    result = validate_reference_data(PROJECT_ROOT)

    coverage = result["formal_reference_coverage"]

    assert result["actions_count"] >= 120
    assert coverage["missing_action_facets"] == []
    assert coverage["missing_goal_groups"] == {}
    assert coverage["missing_risk_levels"] == []
    assert coverage["minimum_counts"]["actions"]["actual"] >= coverage["minimum_counts"]["actions"]["required"]




def test_validate_reference_data_reports_contraindication_coverage_matrix():
    result = validate_reference_data(PROJECT_ROOT)

    coverage = result["formal_reference_coverage"]["contraindication_coverage"]

    assert coverage["minimum_count"] == {"actual": 60, "required": 60, "ok": True}
    assert coverage["missing_required_risk_levels"] == []
    assert coverage["missing_required_types"] == []
    assert coverage["disease_count"] >= 40
    assert coverage["forbidden_action_category_count"] >= 20
    assert coverage["stop_signal_count"] >= 20
    assert coverage["referral_criteria_count"] >= 20
    assert coverage["by_risk_level"]["R2"] > 0
    assert coverage["by_risk_level"]["R3"] > 0
    assert coverage["by_type"]["relative"] > 0
    assert coverage["by_type"]["absolute"] > 0
    assert coverage["missing_required_fields"] == {}

def test_validate_reference_data_accepts_formal_goal_synonyms(tmp_path):
    ...
    action = _action("ACT_ALL", "居家弹力带平衡呼吸训练", "有氧")
    action["suitable_tags"] = ["抗阻", "柔韧", "平衡", "呼吸训练", "低冲击", "居家", "老年友好"]
    action["requires_equipment"] = True
    _write_json(docs_dir / "ai_exercise_action_library_v0_1_expert_review_draft.json", [action])
    _write_json(
        docs_dir / "ai_exercise_prescription_template_library_v0_1_expert_review_draft.json",
        [
            _template("TPL_R0", "R0", ["健康维持", "久坐减少"], ["居家弹力带平衡呼吸训练"]),
            _template("TPL_R1", "R1", ["减脂", "心肺耐力"], ["居家弹力带平衡呼吸训练"]),
            _template("TPL_R2", "R2", ["血糖管理辅助", "血压管理辅助", "肌力提升", "防跌倒"], ["居家弹力带平衡呼吸训练"]),
            _template("TPL_R3", "R3", ["医学评估", "转介建议"], None),
        ],
    )
    _write_json(
        docs_dir / "ai_exercise_prescription_risk_rules_v0_1_expert_review_draft.json",
        [_rule("RULE_R0", "R0", "GREEN"), _rule("RULE_R1", "R1", "GREEN"), _rule("RULE_R2", "R2", "YELLOW"), _rule("RULE_R3", "R3", "RED")],
    )
    _write_json(
        docs_dir / "ai_exercise_compliance_copy_pack_v0_1_expert_review_draft.json",
        [{"code": "SAFE", "title": "安全边界", "text": "R2审核前不下发，R3只转介。"}],
    )
    _write_json(docs_dir / "knowledge_source_catalog_v0_2.json", {"sources": [{"title": "WHO 2020"}]})

    result = validate_reference_data(docs_dir=docs_dir, rag_root=rag_root, strict=False)
    coverage = result["formal_reference_coverage"]

    assert coverage["missing_action_facets"] == []
    assert coverage["missing_goal_groups"] == {}
    assert coverage["missing_risk_levels"] == []
    assert coverage["matched_goal_groups"]["控糖"]
    assert coverage["matched_goal_groups"]["控压"]



def test_validate_reference_data_reports_action_field_completeness_and_conflicts(tmp_path):
    docs_dir = tmp_path / "docs"
    rag_root = tmp_path / "rag_data"
    docs_dir.mkdir()
    (rag_root / "_manifests").mkdir(parents=True)
    ...
    incomplete_action = _action("ACT_BAD", "冲突动作", "有氧")
    incomplete_action.pop("impact_level")
    incomplete_action.pop("joint_stress_level")
    incomplete_action.pop("evidence_refs")
    incomplete_action["suitable_tags"] = ["低冲击", "胸痛"]
    incomplete_action["contraindication_tags"] = ["胸痛"]
    duplicate_action = _action("ACT_BAD", "冲突动作", "抗阻")
    _write_json(docs_dir / "ai_exercise_action_library_v0_1_expert_review_draft.json", [incomplete_action, duplicate_action])
    _write_json(
        docs_dir / "ai_exercise_prescription_template_library_v0_1_expert_review_draft.json",
        [_template("TPL_R1", "R1", ["减脂"], ["冲突动作"])],
    )
    _write_json(
        docs_dir / "ai_exercise_prescription_risk_rules_v0_1_expert_review_draft.json",
        [_rule("RULE_R1", "R1", "GREEN")],
    )
    _write_json(
        docs_dir / "ai_exercise_compliance_copy_pack_v0_1_expert_review_draft.json",
        [{"code": "SAFE", "title": "安全边界", "text": "R2审核前不下发，R3只转介。"}],
    ...
    result = validate_reference_data(docs_dir=docs_dir, rag_root=rag_root, strict=False)
    action_quality = result["formal_reference_coverage"]["action_quality"]

    assert action_quality["missing_required_fields"]["impact_level"] == ["ACT_BAD"]
    assert action_quality["missing_required_fields"]["joint_stress_level"] == ["ACT_BAD"]
    assert action_quality["missing_required_fields"]["evidence_refs"] == ["ACT_BAD"]
    assert action_quality["duplicate_action_codes"] == ["ACT_BAD"]
    assert action_quality["duplicate_action_names"] == ["冲突动作"]
    assert any("suitable/contraindication conflict" in item for item in action_quality["conflicts"])
    assert any("missing required action fields" in item for item in result["blocking_errors"])

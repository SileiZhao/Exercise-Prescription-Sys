...
export function UserDashboardPage() {
  ...
  const profileCompletion = summary?.profile_completion_rate ?? (healthRadarData.length ? "已采集" : "待补齐");
  ...
  return (
    ...
    >
      <Space direction="vertical" size={16} className="onboarding-section user-dashboard-page">
        <section className={`today-safety-band risk-band-${summary?.current_risk_level ?? "unknown"}`}>
        ...
        <Row gutter={[12, 12]}>
          <Col xs={24} sm={12} lg={6}>
            <MetricCard title="档案完成度" value={profileCompletion} icon={<ShieldCheck />} />
          </Col>
          <Col xs={24} sm={12} lg={6}>

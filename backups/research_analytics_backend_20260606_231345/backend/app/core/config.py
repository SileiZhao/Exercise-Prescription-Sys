...
def runtime_provider_summary(config: "Settings") -> dict[str, Any]:
    llm_key, llm_key_name = llm_api_key(config)
    embedding_key, embedding_key_name = embedding_api_key(config)
    return {
        ...
        "llm": {
            ...
            "api_key_name": llm_key_name,
            "api_key": mask_secret(llm_key),
        },
        "embedding": {
            "provider": config.EMBEDDING_PROVIDER.strip().lower(),
            "model": config.EMBEDDING_MODEL,
            "base_url": embedding_base_url(config),
            "api_key_name": embedding_key_name,
            "api_key": mask_secret(embedding_key),
        },
        "ocr": {

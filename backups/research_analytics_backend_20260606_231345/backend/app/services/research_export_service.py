...
class ResearchExportService:
    ...
    def export_users(self, requester: User) -> dict[str, Any]:
        ...
        AuditService(self.db).record(
            ...
            metadata={
                ...
                "desensitized": True,
                "fields": ["participant_code", "profile", "fitness_test", "risk_screening", "latest_prescription"],
            },
        )
        self.db.commit()
    ...
    def summary(self, requester: User) -> dict[str, Any]:
        ...
        return {
            "total_participants": self.db.scalar(
                self._scoped_user_model_select(select(func.count(UserProfile.id)), UserProfile, organization_id)
            )
            or 0,

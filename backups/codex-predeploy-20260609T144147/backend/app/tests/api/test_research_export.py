...
def test_admin_direct_research_export_is_desensitized_and_researcher_direct_export_is_forbidden(
    ...
    assert body["total"] == 1
    row = body["items"][0]
    assert row["research_subject_id"].startswith("SUB-")
    assert row["participant_code"].startswith("R")
    assert row["participant_code"] != "P000001"
    assert row["age_band"] == "30-39"
    assert "age" not in row["profile"]
...
def test_research_summary_reports_desensitized_distribution_and_effects(client: TestClient, db_session):
    ...
    assert body["cluster_risk_overlay"] == {"肥胖代谢风险型": {"R2": 1}}
    assert body["prescription_status"]["PENDING_REVIEW"] == 1
    assert body["template_effects"]["快走"] == 90
    assert body["export_job_status"] == {}
    ...
    assert body["intervention_effects"]["completion_rate_trend"] == [{"date": "2026-05-30", "value": 90}]
    assert body["intervention_effects"]["rpe_trend"] == [{"date": "2026-05-30", "value": 12}]
    assert body["intervention_effects"]["pain_trend"] == [{"date": "2026-05-30", "value": 1}]
...
def test_researcher_cannot_download_same_org_request_owned_by_another_researcher(client: TestClient):
    requester_headers = login(client, "owner-boundary-requester@example.com", "RESEARCHER", organization_id=1)
    peer_headers = login(client, "owner-boundary-peer@example.com", "RESEARCHER", organization_id=1)
    admin_headers = login(client, "owner-boundary-admin@example.com", "ADMIN")

    created = client.post(
        "/api/v1/research/export/requests",
        headers=requester_headers,
        json={"format": "json", "purpose": "同机构申请人边界测试"},
    )
    assert created.status_code == 201
    request_id = created.json()["id"]
    approved = client.post(
        f"/api/v1/research/export/requests/{request_id}/approve",
        headers=admin_headers,
        json={"approval_comment": "批准脱敏导出"},
    )
    assert approved.status_code == 200

    peer_download = client.get(f"/api/v1/research/export/requests/{request_id}/download", headers=peer_headers)
    owner_download = client.get(f"/api/v1/research/export/requests/{request_id}/download", headers=requester_headers)

    assert peer_download.status_code == 403
    assert "another request" in peer_download.json()["detail"]
...
def test_researcher_summary_is_scoped_to_own_organization(client: TestClient, db_session):
    ...
    assert body["cluster_risk_overlay"] == {"机构一分型": {"R1": 1}}

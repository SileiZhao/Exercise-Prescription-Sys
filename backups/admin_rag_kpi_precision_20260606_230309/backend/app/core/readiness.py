...
from app.core.config import Settings, embedding_api_key, embedding_base_url, llm_api_key, runtime_provider_summary, settings
...
class ReadinessService:
    ...
    def check_llm(self) -> ComponentStatus:
        ...
        if provider in {"openai-compatible", "openai", "qwen", "dashscope", "aliyun"}:
            ...
            runtime = runtime_provider_summary(self.settings)
            key_name = runtime["llm"].get("api_key_name") or "LLM_API_KEY"
            masked_key = runtime["llm"].get("api_key")
            return ComponentStatus(
                status="ok",
                detail=f"{provider}:{self.settings.LLM_MODEL} key={key_name}:{masked_key}",
            )
    ...
    def check_embedding(self) -> ComponentStatus:
        ...
        if provider in {"openai-compatible", "openai", "dashscope", "aliyun"}:
            ...
            api_key, key_name = embedding_api_key(self.settings)
            if not api_key:
                return ComponentStatus(status="down", detail="missing EMBEDDING_API_KEY")
            runtime = runtime_provider_summary(self.settings)
            masked_key = runtime["embedding"].get("api_key")
            return ComponentStatus(status="ok", detail=f"{detail} key={key_name}:{masked_key}")

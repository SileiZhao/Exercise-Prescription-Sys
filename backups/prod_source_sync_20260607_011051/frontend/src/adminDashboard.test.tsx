...
const readySummary = {
  ...
  reference_data_status: {
    ...
    knowledge: { documents: 53, skipped_documents: 0, chunks: 12050, indexed_chunks: 12050 },
...
describe("admin dashboard", () => {
  ...
  it("renders operational and safety statistics", async () => {
    ...
    expect(screen.getByText("处方生成数")).toBeInTheDocument();
    expect(screen.getByText("处方发布数")).toBeInTheDocument();
    expect(screen.getByText("专家审核数")).toBeInTheDocument();
    expect(screen.getByText("R2平均审核时长")).toBeInTheDocument();
  ...
  it("renders blocking readiness reasons for LLM OCR and RAG index gaps", async () => {
    getAdminDashboardSummaryMock.mockResolvedValueOnce({
      ...
      reference_data_status: {
        ...
        knowledge: { documents: 53, skipped_documents: 3, chunks: 12050, indexed_chunks: 8000 },
    ...
    expect(screen.getByText("RAG 索引未完成：8000 / 12050 切片")).toBeInTheDocument();
  });
});
